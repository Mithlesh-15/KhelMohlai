...
function LoadingCard() {
  return (
    <div className="surface-card animate-pulse">
      <div className="flex items-center justify-between gap-3">
        {[0, 1].map((item) => (
          <div key={item} className="flex min-w-0 flex-1 items-center gap-3">
            <div className="h-12 w-12 rounded-2xl" style={{ backgroundColor: 'var(--skeleton)' }} />
      ...
      <div className="mt-5 flex items-center justify-between gap-3 border-t pt-4" style={{ borderColor: 'rgba(217, 226, 236, 0.9)' }}>
        <div className="h-4 w-32 rounded-full" style={{ backgroundColor: 'var(--skeleton)' }} />
        <div className="h-7 w-24 rounded-full" style={{ backgroundColor: 'var(--skeleton)' }} />
      </div>
    </div>
  );
}
  ...
  return (
    <main className="px-4 pb-28 pt-4 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-2xl flex-col gap-5">
        ...
        {!isLoading && isError ? (
          <section className="feedback-panel" style={{ backgroundColor: '#fffaf9', borderColor: 'rgba(248, 113, 113, 0.24)', color: 'var(--text-primary)' }}>
